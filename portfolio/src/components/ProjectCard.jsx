...
/* ─── Card thumbnail gradient per category ─────────────────────────────────── */
const CAT_GRAD = {
...
export function ProjectCard({ project, onClick, onLiveDemo, onViewWorkflow }) {
  return (
    <motion.div variants={cardHover} initial="rest" whileHover="hover"
      className="relative rounded-2xl group/glow">
      {/* Animated gradient border glow — sits behind the card, fades in on hover */}
      <div
        aria-hidden="true"
        className="absolute -inset-px rounded-2xl opacity-0 group-hover/glow:opacity-100 transition-opacity duration-500 pointer-events-none"
        style={{
          background: 'linear-gradient(120deg, #3B82F6, #8B5CF6, #06B6D4, #3B82F6)',
          backgroundSize: '300% 300%',
          animation: 'border-glow-sweep 4s linear infinite',
          filter: 'blur(6px)',
        }}
      />
      <div className="relative glass rounded-2xl overflow-hidden cursor-pointer flex flex-col"
      ...
      <div className={`relative h-44 bg-gradient-to-br ${CAT_GRAD[project.category] || 'from-blue-900/40 to-purple-900/40'} overflow-hidden`}
        ...
        {project.image ? (
          <img src={project.image} alt={project.title}
            loading="lazy"
            className="w-full h-full object-cover opacity-70 group-hover/glow:opacity-90 group-hover/glow:scale-105 transition-all duration-500"
      ...
      <div className="p-5 flex flex-col flex-1 space-y-3" onClick={() => onClick(project)}>
        ...
        <h3 className="font-bold text-base text-white group-hover/glow:gradient-text transition-all leading-snug">
          {project.title}
        </h3>
    ...
    <AnimatePresence>
      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto glass rounded-2xl border border-white/10"
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <RippleButton onClick={() => { onClose(); onLiveDemo?.(); }}
              ...
              <RippleButton onClick={() => { onClose(); onViewWorkflow?.(); }}
