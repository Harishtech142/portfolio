...
function WFNode({ node }) {
  ...
  return (
    <motion.div
      ...
      style={{ position:'absolute', left: node.x, top: node.y, width: 140, willChange: 'transform' }}
    >
      <motion.div
...
export default function WorkflowCanvas({ nodes, connections, hint = 'Drag to pan · Use the zoom controls' }) {
  ...
  // Touch panning support for mobile/tablet viewports
  useEffect(() => {
    ...
    const handleTouchStart = (e) => {
      if (e.touches.length !== 1) return; // Only pan on single-finger touch
    ...
    const handleTouchMove = (e) => {
      if (!isTouchDragging || !touchLastPos || e.touches.length !== 1) return;
      if (e.cancelable) e.preventDefault(); // Prevent page scrolling while dragging

      const touch = e.touches[0];
      const dx = touch.clientX - touchLastPos.x;
      const dy = touch.clientY - touchLastPos.y;
